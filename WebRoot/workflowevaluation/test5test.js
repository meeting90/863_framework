...
var endp = [];
	...
	var widthofrect = 150, heightofrect = 60, gap = 30, heightofdiamond = 30, widthofdiamond = 20, rectcorner = 5, arrowwidth= 5, crossradius = 10, textheighttomiddle = 20, valuetotop = 40;
	...
	function draw(raph, x, y) {
		...
		for ( var i = 0, ii = raph.childNodes.length; i < ii; i++) {
			if (raph.childNodes[i].name != "invoke"){
					infotext = r.text(x + widthofrect / 2, y + heightofrect / 2,printattr(raph.childNodes[i],false)).attr({"font-size" : 14,fill : fontcolor});
					infotextfull = r.text(x + widthofrect / 2, y + heightofrect / 2,printattr(raph.childNodes[i],true)).attr({"font-size" : 14,	fill : fontcolor});
					infotextfull.hide();	
			}
			else{
				infotext = r.text(x + widthofrect / 2, y + heightofrect / 2 - textheighttomiddle,printattr(raph.childNodes[i],false)).attr({"font-size" : 14,fill : fontcolor});
			...
			if (raph.childNodes[i].name == "if" || raph.childNodes[i].name == "elseif" || raph.childNodes[i].name == "else") {
				...
				for(var j = 1, jj = raph.childNodes[i].childNodes.length; j < jj; ++j){
					if(raph.childNodes[i].childNodes[j].name == "invoke"){
						...
						var infotext2 = r.text(x+(j-1)*(widthofrect + gap)+ widthofrect / 2, (y -horizontal_gap/2* (gap + heightofrect)+ heightofrect / 2 ) - textheighttomiddle,printattr(raph.childNodes[i].childNodes[j],false)).attr({"font-size" : 14,fill : fontcolor});
...
//				sum += drawAnalysis(raph.childNodes[i],x + ( sum)*4*anaGap,y);
	...
	function drawgraph(divid,bpeljson,wid,hght,px,py){
		...
		r = Raphael(divid, canvaslength, canvaswidth);
		  ...
		  istrust = false;
		  process(text);
		  widthofrect = canvaslength/depth(text)/1.4;
	  	  heightofrect = canvaswidth/height(text)/2;
