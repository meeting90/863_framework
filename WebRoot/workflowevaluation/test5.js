	...
	function draw(raph, x, y) {
		...
		for ( var i = 0, ii = raph.childNodes.length; i < ii; i++) {
			if (raph.childNodes[i].name != "invoke"){
				...
				service = printattr(raph.childNodes[i],false);
				infotext = r.text(x + widthofrect / 2, y + heightofrect / 2 - textheighttomiddle,service.substring(0,service.length-7)).attr({"font-size" : 11,fill : fontcolor});
				infotextfull = r.text(x + widthofrect / 2, y + heightofrect / 2 - textheighttomiddle,service).attr({"font-size" : 14,fill : fontcolor});
				infotextfull.hide();
			...
			if (raph.childNodes[i].name == "if" || raph.childNodes[i].name == "elseif" || raph.childNodes[i].name == "else") {
				...
				for(var j = 1, jj = raph.childNodes[i].childNodes.length; j < jj; ++j){
					if(raph.childNodes[i].childNodes[j].name == "invoke"){
						...
						var service2 = printattr(raph.childNodes[i].childNodes[j],false);
						var infotext2 = r.text(x+(j-1)*(widthofrect + gap)+ widthofrect / 2, (y -horizontal_gap/2* (gap + heightofrect)+ heightofrect / 2 ) - textheighttomiddle,service2.substring(0,service2.length-7)).attr({"font-size" : 11,fill : fontcolor});
...
//		}
		...
		for (var i=0,ii= raph.childNodes.length;i <ii;++i){
			...
			else if (raph.childNodes[i].name == 'if' || raph.childNodes[i].name == 'elseif'){
				indexSwitch = i;
				countSwitch ++;
				sum += drawAnalysis(raph.childNodes[i],x + ( sum)*4*anaGap,y);
				paintIndex++;
				indexElse = raph.childNodes[i].childNodes.length -1;
			}
//			else
//				sum += drawAnalysis(raph.childNodes[i],x + ( sum)*4*anaGap,y);
		}
		if(countSwitch != 0) 
			countSwitch ++;
		if (indexSwitch != -1){
				sum += drawAnalysis(raph.childNodes[indexSwitch].childNodes[indexElse],x + ( sum)*4*anaGap,y);
				paintIndex ++;
				...
				var ttrustSel =0;
				for(var k =0, kk = countSwitch;k<kk;++k){
					var endp = pend.pop();
		...
		if (raph.childNodes.length > 1){
			...
			var ttrustSeq = 1;
			if (raph.name == "if" || raph.name == "elseif"){
				for(var k =1, kk = raph.childNodes.length -1 ;k<kk;++k){ //escape 1st "condition" and last "else"
					 endp = pend.pop();
	...
	function drawgraph(divid,bpeljson,wid,hght,px,py){
		...
		widthofrect = canvaslength/(depth(text)+1)/1.5;
	  	heightofrect = canvaswidth/(height(text)+1)/2;
	  	//console.info("widthofrect:"+widthofrect);
	  	//console.info("canvaswidth:"+canvaswidth);	  	
	  	//console.info("heightofrect:"+heightofrect);
	    while (heightofrect > canvaswidth/8) {
	    	  heightofrect = heightofrect/2;
	    	 
	    }
	    while (widthofrect > canvaslength/6) 
	     widthofrect = widthofrect /1.5;
	    if (heightofrect > widthofrect) 
	    	  heightofrect = widthofrect /2;
	  	  gap = heightofrect /2;
	  	  heightofdiamond = heightofrect/2;
	  	  rectcorner = widthofrect / 24;
	  	  ...
	  	  //console.info("before draw");
		  draw(text, positionx, positiony);
		  //console.info("after draw");
		  linklines();
	}
