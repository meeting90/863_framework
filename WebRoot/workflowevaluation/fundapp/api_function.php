...
function showWorkflowEvaluation($uid, $workflow_name){
  $rootNode = searchBpel($workflow_name);
  //var_dump($rootNode);
  if($rootNode){
...
function showBpelServices($workflow_name){
  $rootNode = searchBpel($workflow_name);
//  var_dump($rootNode);
  $services = array();
...
function showBpelJson($workflow_name){
  $rootNode = searchBpel($workflow_name);
...
function searchBpel($workflow_name) {
	$bfs = new Model('bpel_files');
  ...
  //  var_dump( $bpel->children($ns['bpel']) );
    search($workflow_name, $ns['bpel'], $bpel->children($ns['bpel'])->sequence, $rootNode);
    return $rootNode;
  }
  return $false;
}
function search($workflow_name, $namespace, $rootElement, $rootNode) {
...
// 	    echo $servname."<br/>";
 	    $eps = getEvaParams($_SESSION['uid'], $servname);
...
//					echo $child->getName() . $child->count() . "<br />";
	  if($child->count() > 0){
	    if($child->getName() == "sequence")
	      search($workflow_name, $namespace, $child, $rootNode);
	    else 
	      search($workflow_name, $namespace, $child, $node);
	  }
	}
  }
}
