<?php
	...
	if($apiname == "showworkflow"){
		...
		if(isset($_GET['workflowname']) && $_GET['workflowname'] != ''){
			echo showBpelJson($_GET['uid'],$_GET['workflowname']);
		} else {
			echo "no file specified to show" . "<br/>";
		}
		...
		if(isset($_GET['workflowname']) && $_GET['workflowname'] != ''){
			echo showWorkflowEvaluation($_GET['uid'], $_GET['workflowname']);
		} else {
			echo "no file specified to show" . "<br/>";
		}
...
//		echo showAllBpelFiles();
    ...
    echo showBpelServices($_GET['uid'],$_GET['workflowname']);

  } else if($apiname == "servforcastinfo"){
   	echo showServForcastInfo(getServiceByName($_GET['servname'])->sid, $_GET['uid']);
...
//	echo "please specify api name, try /api.php?apiname=help"; 
	...
	echo 'api.php?apiname=getgraphlist&uid=1</br>获取该用户graph列表</br>';
	echo 'api.php?apiname=addgraph&uid=1</br>增加graph</br>';
	echo 'api.php?apiname=graph&graphid=1&uid=1</br>显示原始graph</br>';
	echo 'api.php?apiname=subgraph&graphid=1&source=37&target=87&uid=1</br>获取经过ks抽取的子graph的json数据</br>';
	echo 'api.php?apiname=evograph&graphid=1&source=36&target=87&edgenum=5&uid=1</br>获取经过evo抽取的最终结果的json数据</br>';

		echo "/api.php?apiname=listworkflow&uid=1<br/>列出所有的工作流<br/><br/>";
//	  echo "/api.php?apiname=listbpel<br/>列出所有的bpel文件名<br/><br/>";
	  echo "/api.php?apiname=showworkflow&workflowname=Travel&uid=1<br/>获取一个工作流的内容<br/><br/>";
	  echo "/api.php?apiname=showwsdl&wsdlname=xxx.wsdl&uid=1<br/>获取一个wsdl文件的内容<br/><br/>";
	  echo "/api.php?apiname=workflow2json&workflowname=Travel&uid=1<br/>一个工作流的信息以json格式返回<br/><br/>";
	  echo "/api.php?apiname=listservice&uid=1<br/>列出数据库里所有的服务<br/><br/>";
    echo "/api.php?apiname=listworkflowservice&workflowname=Travel&uid=1<br/>列出一个工作流里所有的服务<br/><br/>";
	  echo "/api.php?apiname=getservdata&servname=WeatherForecastServiceService&uid=1<br/>服务的观测数据<br/><br/>";
	  echo "/api.php?apiname=servforcast&servname=WeatherForecastServiceService&uid=1<br/>计算一个用户对某个服务的单点服务评估值<br/><br/>";
	  echo "/api.php?apiname=evaluate&workflowname=Travel&uid=1<br/>工作流的评估值<br/><br/>";

	  echo "/api.php?apiname=evaparamupdate&uid=1&sname=WeatherForecastServiceService&threshold=50&lambda=0.8&uid=1<br/>更新用户的评估参数<br/><br/>";
	  echo "/api.php?apiname=evaparaminfo&uid=1&sname=WeatherForecastServiceService&uid=1<br/>获取用户的评估参数<br/><br/>";
    echo "/api.php?apiname=setbackupservice&workflowname=Travel&mainservice=WeatherForecastServiceService&backupservice=WeatherForecastService2Service&uid=1<br/>更新服务的备选服务<br/><br/>";    
    echo "/api.php?apiname=getbackupservice&workflowname=Travel&mainservice=WeatherForecastServiceService&uid=1<br/>获取服务的备选服务列表<br/><br/>";
    echo "/api.php?apiname=addservice&uid=1<br/>用于处理添加服务的表单的页面,表单提交如下内容:POST['service_name'],FILE['wsdl']<br/><br/>";
    echo "/api.php?apiname=updateserviceaddress&servicename=xxx&address=yyy&uid=1<br/>更新服务的地址信息,yyy是url编码的串<br/><br/>";
    ...
    echo "/api.php?apiname=addvisitlog&name=xxx&time=yyy&log=lll&uid=1<br/>添加工作流访问记录信息,xxx为url格式的数据<br/><br/>";
    echo "/api.php?apiname=getvisitlog&name=xxx&uid=1<br/>获取工作流访问记录信息<br/><br/>";
    echo "/api.php?apiname=getbackupservicelist&workflowname=Travel&uid=1<br/>获取备选服务LIST<br/>";
    echo "/api.php?apiname=run&workflowname=Travel&uid=1<br/>";
    echo "tips:</br>";
